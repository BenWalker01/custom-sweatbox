use crate::config::{
    DepartureRoute, ProfileConfig, StandardDeparture, StandardTransit, TransitRoute,
};
use anyhow::Result;
...
impl Scenario {
    ...
    pub fn master_controller(&self) -> (&str, &str) {
        (
            &self.config.master_controller,
            &self.config.master_controller_freq,
        )
    }
    ...
    pub fn departure_destinations(&self) -> Vec<&str> {
        let mut destinations: Vec<&str> = self
            .config
            .std_departures
            .iter()
    ...
    pub fn statistics(&self) -> ScenarioStats {
        let total_departure_routes: usize = self
            .config
            .std_departures
            .iter()
            .map(|d| d.routes.len())
        ...
        let total_transit_routes: usize = self
            .config
            .std_transits
            .iter()
            .map(|t| t.routes.len())
...
impl std::fmt::Display for ScenarioStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  Departure Configurations: {} ({} routes)",
            self.departure_configs, self.total_departure_routes
        )?;
        writeln!(
            f,
            "  Transit Configurations: {} ({} routes)",
...
mod tests {
    ...
    fn test_random_routes() -> Result<()> {
        ...
        if let Some(route) = dep_route {
            println!(
                "Random departure from EGSS: {} -> {}",
                route.route, route.arriving
            );
        }
